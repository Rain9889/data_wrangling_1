...
Choose some columns and others
...
``` r
select(litters_df, GROP = group, LITTer_NUmBer = litter_number)
```
...
``` r
rename(litters_df, GROP = group, LITTer_NUmBer = litter_number)
```

    ## # A tibble: 49 × 8
    ##    GROP  LITTer_NUmBer   gd0_weight gd18_weight gd_of_birth pups_born_alive
...
litter_without_missing = drop_na(litters_mutated, gd0_weight)
```
